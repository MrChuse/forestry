...
class BestiaryWindow(UIWindow):
    ...
    def rebuild(self):
        ...
        self.table.table_contents.append([
            UILabel(pygame.Rect(0, 0, 100, 30), local['species'], container=self.table),
            UILabel(pygame.Rect(0, 0, 100, 30), local['produced'], container=self.table),
            UILabel(pygame.Rect(0, 0, 100, 30), local['resource'], container=self.table),
            UILabel(pygame.Rect(0, 0, 100, 30), local['produced'], container=self.table)
        ])
        ...
        for known_bee_species in self.bestiary.known_bees:
            ...
            for product, amount in self.bestiary.produced_resources[known_bee_species].items():
                product_labels.append(UILabel(pygame.Rect(0, 0, 100, 30), str(product), container=self.table))
                product_labels.append(UILabel(pygame.Rect(0, 0, 100, 30), str(amount), container=self.table))
            ...
            self.table.table_contents.append([
                ...
                UILabel(pygame.Rect(0, 0, 100, 30), str(self.bestiary.known_bees[known_bee_species]), container=self.table)
            ] + product_labels)
        self.table.rebuild()
